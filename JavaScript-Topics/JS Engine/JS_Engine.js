...
A.  JRE is an environment in which the JS code is executed.
...
A. 
JS Engine is the heart of JRE where the JS code is compiled and executed.
...
A.  JS is a JIT compiled Language(Just-In-Time).
    ...
    Interpreter converts the AST into bytecode , which is ready to be executed
    Interpreter works closely with the compiler. 
    The compiler "optimizes the bytecode at RUNTIME".

    Because the compiler optimizes the bytecode at RUNTIME working with the interpreter, this is why it's called JUST-IN-TIME compilation.

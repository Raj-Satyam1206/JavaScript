...
With callback functions, we were getting into Callback Hell & Inversion of control.
...
/*
    ...
    Promises give us the guarantee that the callback function will be executed exactly ONCE and the control remains with the developer/user 
*/
...
/*
    ...
    When the promise is created, it will be in the pending state and after some time, when the response has been received from async operation, then STATE becomes 'fulfilled' / 'rejected' and RESULT changes to the response object.
...
promise
  .then((response) => proceedToPayment(response));
...
createOrder(cart)
 ...
 .then((paymentInfo) => getSummary(paymentInfo))
 .then(() => updateWallet())
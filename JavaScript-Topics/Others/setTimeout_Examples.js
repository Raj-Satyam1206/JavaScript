...
Asynchronous Operation was ready to be executed after 5s, but it will still wait for 10s for the entire program to end. 
...
- Then while loop run for 10 seconds and by then timer of 5s for setTimeout expires and ready to be executed.
...
// The callback functions in the callback queue or microtask queue will only be executed after the entire program has been executed synchronously and GEC is popped out from call stack. Then the asynchronous tasks will be executed.
...
When we want that code to be executed later and not synchronously, then we can put a timer of 0s, so that it will move to callback queue and wait until the program ends.
*/
